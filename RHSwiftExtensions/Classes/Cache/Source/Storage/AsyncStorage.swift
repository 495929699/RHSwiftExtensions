...
extension AsyncStorage {
  ...
  public func object(forKey key: String, completion: @escaping (CacheResult<T>) -> Void) {
    entry(forKey: key, completion: { (CacheResult: CacheResult<Entry<T>>) in
      completion(CacheResult.map({ entry in
        return entry.object
      }))
    })
  }
  ...
  public func existsObject(
    ...
    object(forKey: key, completion: { (CacheResult: CacheResult<T>) in
      completion(CacheResult.map({ _ in
        return true
      }))
    })
  }
